...
export default function AdminAcneSolutions() {
  ...
  const handleDelete = (id: number, type: string) => {
    if (confirm(`Apakah Anda yakin ingin menghapus rekomendasi jerawat "${type}"?`)) {
        ...
        <div className="max-w-7xl mx-auto flex flex-col pb-32 md:pb-8">
          ...
          <div className="bg-[var(--dashboard-card-bg)] rounded-3xl p-6 md:p-8 shadow-[0_10px_40px_rgba(0,0,0,0.03)] border border-[var(--dashboard-border)] transition-colors duration-300">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
              <div>
                <h1 className="text-2xl font-black tracking-tight uppercase text-[var(--dashboard-text)]">Master Data: Rekomendasi</h1>
                <p className="text-sm font-medium text-on-surface-variant/70 mt-1">Kelola data Acne Solutions yang akan disarankan pada pengguna.</p>
              </div>
              <button onClick={handleAddNew} className="bg-primary text-on-primary font-bold px-6 py-3 rounded-xl hover:bg-primary/90 transition-colors flex items-center justify-center gap-2">
            ...
            {isLoading ? (
              <div className="animate-pulse flex flex-col gap-4">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="h-24 bg-[var(--dashboard-border)] rounded-xl"></div>
                ))}
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {solutions?.map((solution: any) => (
                  <div key={solution.id} className="bg-[var(--dashboard-bg)] border border-[var(--dashboard-border)] rounded-2xl p-5 hover:shadow-lg transition-all duration-300 group">
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => handleEdit(solution)} className="w-8 h-8 rounded-full bg-[var(--dashboard-border)] flex items-center justify-center text-on-surface-variant hover:text-blue-500 transition-colors">
                          <span className="material-symbols-outlined text-[16px]">edit</span>
                        </button>
                        <button onClick={() => handleDelete(solution.id, solution.type)} disabled={deleteMutation.isPending} className="w-8 h-8 rounded-full bg-[var(--dashboard-border)] flex items-center justify-center text-on-surface-variant hover:text-red-500 transition-colors disabled:opacity-50">
                          <span className="material-symbols-outlined text-[16px]">delete</span>
                        </button>
                      </div>
                    </div>
                    <h3 className="text-lg font-extrabold text-[var(--dashboard-text)] mb-2 capitalize">{solution.type}</h3>
