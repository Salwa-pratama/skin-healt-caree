            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
              <div>
                ...
                <p className="text-sm font-medium text-on-surface-variant/70 mt-1">
                  Kelola data seluruh akun terdaftar di aplikasi.
                </p>
              </div>
            </div>
            ...
            {isLoading ? (
              <div className="animate-pulse flex flex-col gap-4">
                {[1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="h-16 bg-[var(--dashboard-border)] rounded-xl"
                  ></div>
                ))}
              </div>
            ) : (
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody>
                    {users?.map((user: any) => (
                      <tr
                        key={user.id}
                        className="border-b border-[var(--dashboard-border)] hover:bg-[var(--dashboard-bg)] transition-colors"
                      >
                        ...
                        <td className="py-4 px-4 text-sm font-medium text-on-surface-variant">
                          {user.email}
                        </td>
                        <td className="py-4 px-4">
                          <span
                            className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${user.role === "admin" ? "bg-primary-container text-on-primary-container" : "bg-surface-variant text-on-surface-variant"}`}
                          >
                            {user.role}
                          </span>
                        </td>
